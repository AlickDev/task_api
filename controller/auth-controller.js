...
const logger = log4js.getLogger("Authentication");
...
exports.login = async (req, res, next) => {
  try {
    ...
    const login = await userRepo.loginUser(
      email,
      password,
      latitude,
      longitude,
    ...
    // Save log
    logger.error("Error", error, "IP:", req.ip);

    if (error.message === "Invalid username or password") {
      res.status(400).json({ error: "Invalid username or password" });
    } else if (
      error.message === "User is not active or is a workspace" ||
      error.message === "Company is not active"
    ) {
